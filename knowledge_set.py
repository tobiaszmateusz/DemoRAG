...
def compute_knowledge_vectorstore(name: str, embeddings, base_path: str = "data"):
    ...
    if os.path.exists(os.path.join(full_path, "faiss_index")):
        vectorstore = Chroma.load_local(os.path.join(full_path, "faiss_index"), embeddings=embeddings)
    ...
    if missing_docs:
        new_vectorstore = Chroma.from_documents(missing_docs, embedding=OpenAIEmbeddings())
